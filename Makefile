...
src_MEM_AST_DUMPER:=`echo src/mem/ast/dumper/*.cpp`
...
memc: node_types tokenizer parser
	$(GXX) -O0 src/mem-compile.c src/mem/Compiler.cpp src/mem/Util.cpp ${src_MEM_AST} ${src_MEM_AST_NODE} ${src_MEM_AST_DUMPER} ${src_MEM_FS} ${src_MEM_FS_POSITION} ${src_MEM_LOG} ${src_MEM_PARSER} ${src_MEM_TOKENIZER} ${src_MEM_ST} -o build/mem-compile -I./src

thomas_memc: memc
	ln -f ./build/mem-compile ~/bin/

mem-tokenize: node_types tokenizer parser
	${GXX} -o build/mem-tokenize  src/mem-tokenize.c ${src_MEM_AST} ${src_MEM_AST_NODE} ${src_MEM_AST_DUMPER} ${src_MEM_FS} ${src_MEM_FS_POSITION} ${src_MEM_LOG} ${src_MEM_PARSER} ${src_MEM_TOKENIZER} ${src_MEM_ST}
